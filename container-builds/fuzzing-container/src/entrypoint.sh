...
# : "${FUZZ_MEMORY:=2g}"          # total memory budget — available but unused here
...
: "${FUZZ_RSS_LIMIT:=2048}"               # per-process RSS limit in MB
...
do_coverage() {
	...
	log "Coverage outputs: cov-html/, coverage.json, coverage.lcov"
	exit 0
}
