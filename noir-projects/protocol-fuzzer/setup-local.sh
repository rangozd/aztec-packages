...
# Symlink wallet-bridge.mjs into yarn-project/ so @aztec/* resolves from its node_modules.
# --preserve-symlinks(-main) makes Node resolve from the symlink location, not the target.
ln -sf "../noir-projects/protocol-fuzzer/wallet-bridge.mjs" "${REPO_ROOT}/yarn-project/wallet-bridge.mjs"

log "Starting bridge server on port 8089..."
(cd "${REPO_ROOT}/yarn-project" && exec node --preserve-symlinks --preserve-symlinks-main --no-warnings wallet-bridge.mjs > /tmp/bridge-local.log 2>&1) &
BRIDGE_PID=$!
BG_PIDS+=("$BRIDGE_PID")
